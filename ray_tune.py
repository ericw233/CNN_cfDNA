...
def ray_tune(num_samples=200, max_num_epochs=1000, gpus_per_trial=1, 
    ...
    config = {
        ...
        "conv1": tune.choice([i for i in range(1,3)]),
        "pool1": tune.choice([i for i in range(1,3)]),
        "drop1": tune.choice([(i)/5 for i in range(3)]),
        
        "conv2": tune.choice([i for i in range(1,3)]),
    ...
    best_trained_model = CNN(input_size=input_size, num_class=2, 
    ...
    torch.save(best_trained_model,f"{output_path}{feature_type}_CNN_2D_BestRayTune.pt")
